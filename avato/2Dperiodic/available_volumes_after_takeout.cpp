...
{
  FT init_prec = 1.0e-12;
  FT term_prec = 1.0e4;
  bool allow_locate_to_fail = false;

  std::string myname(basename(argv[0]));
  // avato or fv ? <<<
  bool only_one_cavity = (myname.substr(0,12) == "free_volumes" or myname.substr(0,2) == "fv");
  if (only_one_cavity)
  {
    std::cout << "Taking only the one cavity of the omitted particle (free volume)." << std::endl;
  } else {
    std::cout << "Taking all cavities after omitting one particle." << std::endl;
  } // >>>
  ...
  for (int i=1; i<argc; i++)
  {
    ...
    std::string outname = dumpname.substr(0, dumpname.find_last_of(".")) + (only_one_cavity ? ".fv" : ".avato") + (mono ? "_mono" : "") + (shuffle ? "_shuffle" : "");
    struct stat s;
    ...
    TV.treat_available_volumes_after_takeout(only_one_cavity, true, outfile, init_prec, term_prec, allow_locate_to_fail);
    outfile.close();
